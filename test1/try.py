...
def paint_one_excel(point_file_path, line_file_path, final_name):
    ...
    for index, line in enumerate(range(point_excel.shape[0])):
        ...
        m_plot.text(x, 0.95, compound_class, rotation="vertical", horizontalalignment="center", verticalalignment="top")
        ...
        m_plot.text(x, 3.05, compound_class, rotation="vertical", horizontalalignment="center")
    ...
    for index, line in enumerate(range(line_excel.shape[0])):
        ...
        target_x = all_classes.index(target_class) *x_ratio
    ...
    m_plot.axis("off")
    # y设置标题位置
    m_plot.title("%s_%s" % (final_name.title(), "Positive"), y=1.15)
    ...
    for index, line in enumerate(range(point_excel.shape[0])):
        ...
        color = "y" if class_CN_dict[compound_class] == "N" else "b"
        m_plot.scatter(x, 1, color=color, linewidths=line_weight_count / 20, marker="|")
        m_plot.text(x, 0.95, class_index_dict[compound_class], rotation="vertical",
                    horizontalalignment="center", verticalalignment="top")
        m_plot.scatter(x, 3, color=color, linewidths=line_weight_count / 20, marker="|")
        m_plot.text(x, 3.05, class_index_dict[compound_class], rotation="vertical",
    ...
    # 坐标轴不显示，
    m_plot.axis("off")
    m_plot.title("%s_%s" % (final_name.title(), "Negative"),  y=1.15)

    # 边框调整
    m_plot.subplots_adjust(top=0.7, bottom=0.3, left=0.1, right=0.9, hspace=0, wspace=0)

    m_plot.savefig("./try_out_data/%s.eps" % (final_name.title()))
    # 清除所有之前的信息；
    m_plot.clf()
...
for dir in treat_dirs:
    ...
    paint_one_excel(point_file_path, line_file_path, files[0].split("_")[0])
    paint_one_excel(point_file_path, line_file_path, files[0].split("_")[0])
